  ...
  actions: {
    ...
    updateProduct(productId, updatedProduct) {
      ...
      if (index !== -1) {
        ...
        // Recompute aggregate
        const aggregateUnitsSold = Object.values(unitsSoldByColor).reduce(
          (a, b) => a + b,
          existing.unitsSold || 0,
        )
    ...
    updateProductStock(productId, newStock, color) {
      ...
      if (index !== -1) {
        ...
        if (!this.productList[index].stockByColor) {
          ...
          if (this.productList[index].stock !== undefined) {
            const stockPerColor = Math.floor(
              this.productList[index].stock / (this.productList[index].colorVariants?.length || 1),
            )
            ...
            this.productList[index].colorVariants?.forEach((c) => {
              this.productList[index].stockByColor[c] =
                this.productList[index].stockByColor[c] || 50 // Default stock per color
            })
          }
        }
        ...
        if (color && this.productList[index].colorVariants?.includes(color)) {
          ...
          this.productList[index].stockByColor[this.productList[index].colorVariants[0]] = Math.max(
            0,
            newStock,
          )
        }
